...
const Facts = require('../models/facts.model')
    ...
    const validationForm = require('../validations/facts.validation')
    
class FactsController {
    getAll = () => {
        return (req, res, next) => {
            Facts.getAll().then(data => {
    ...
    report = () => {
        return (req, res, next) => {
            Facts.report(req.query).then(data => {
                res.status(200).json(data)
            }).catch(err => {
                res.status(400).json(err)
            })
    ...
    getByColumn = () => {
        return (req, res, next) => {
            ...
            Facts.getByColumn(column, value).then(data => {
                res.status(200).json(data)
            }).catch(err => {
                res.status(400).json(err)
            })
    ...
    createOne = () => {
        return (req, res, next) => {
            ...
            if (validation.error) {
                ...
                Facts.createOne(data).then(data => {
    ...
    updateByID = () => {
        return (req, res, next) => {
            ...
            if (validation.error) {
                ...
                Facts.updateByID(data, id).then(data => {
                    res.status(200).json({new_data : data})
                }).catch(err => {
                    res.status(400).json(err)
                })
        ...
        return (req, res, next) => {
            ...
            if (validation.error) {
                ...
                if (req.query.column || req.query.value) {
                    const column = req.query.column
                    const value = req.query.value
                    Facts.updateByColumn(data, column, value).then(data => {
                        res.status(200).json({new_data : data})
                    }).catch(err => {
                        res.status(400).json(err)
                    })
    ...
    createList = () => {
        return (req, res, next) => {
            ...
            Facts.createList(list).then(data => {
                res.status(200).json()
            }).catch(err => {
                res.status(400).json(err)
            })
