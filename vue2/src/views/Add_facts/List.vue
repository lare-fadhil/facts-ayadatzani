
<template>
    <div>
            <v-container class="my-2">
                    <v-layout row wrap px-0 mb-4>
                        <v-flex xs12>
                            <v-card class="mx-1 px-0" style="background-color:transparent!important" flat>
                                <v-card-title class="px-0">
                                    <v-breadcrumbs class="px-0" :items="$store.getters.language.data.add_facts.list_path">
                                        <template v-slot:item="{ item }">
                                            <v-breadcrumbs-item :to="item.path" :disabled="item.disabled">
                                                {{ item.title.toUpperCase() }}
                                            </v-breadcrumbs-item>
                                        </template>
                                    </v-breadcrumbs>
                                    <v-spacer></v-spacer>
                                    <!-- <v-btn class="float-right" color="primary" icon  @click="exportExcel">
                                        <v-icon>mdi-file-excel</v-icon>
                                    </v-btn> -->
                                    <v-btn class="float-right" color="primary" icon  to="/add-new-add_facts">
                                        <v-icon>mdi-plus</v-icon>
                                    </v-btn>
                                </v-card-title>
                            </v-card>
                        </v-flex>
                    </v-layout>
                <form @submit.prevent="addAddFacts" autocomplete="off">
                    <v-layout row wrap>
        
                    <v-flex xs12 lg2 xl2 md3 sm4>
                        <v-text-field v-model="add_facts.add_facts_text" type="text" :label="$store.getters.language.data.add_facts.add_facts_text" dense
                            class="mx-1"  filled outlined  required>
                        </v-text-field>
                    </v-flex>
                
                        <v-flex xs12 lg2 xl2 md3 sm4>
                                <v-select class="mx-1" clearable  :items="categories" v-model="add_facts.category_id" dense  filled outlined  item-text="category_id"
                                    item-value="category_id" :return-object="false" :label="$store.getters.language.data.categories.category_id">
                                </v-select>
                        </v-flex>
                
                        <v-flex xs12 lg2 xl2 md3 sm4>
                                <v-select class="mx-1" clearable  :items="['true','false']" v-model="add_facts.add_fact_review" dense  filled outlined  item-text="add_fact_review"
                                    item-value="add_fact_review" :return-object="false" :label="$store.getters.language.data.add_facts.add_fact_review">
                                </v-select>
                        </v-flex>
                
                   <!-- <v-flex xs12 lg2 xl2 md3 sm4>
                        <v-text-field v-model="add_facts.add_fact_date" type="datetime-local" :label="$store.getters.language.data.add_facts.add_fact_date" dense
                            class="mx-1"  filled outlined >
                        </v-text-field>
                    </v-flex> -->
                        <v-flex xs12 lg2 xl2 md2 sm4>
                            <v-btn  color="primary" :loading="loading_btn" type="submit" >{{$store.getters.language.data.add_facts.add_btn}}</v-btn>
                        </v-flex>
                    </v-layout>
                </form>
<v-layout row wrap mt-5>
    <v-flex xs12>
        <v-card flat>
            <v-card-text>
                <v-data-table :loading="loading" :headers="headers"  show-select v-model="selected_rows"    :search="search" :items="rows" class="elevation-0"
                    item-key="add_fact_id">
                    <template v-slot:[`item.add_fact_id`]="{ item }">
                        <div>
                            <v-btn icon :to="'/add_facts-list/'+item.add_fact_id"  color="teal" class="mx-1">
                                <v-icon> mdi-pencil-outline </v-icon>
                            </v-btn>
                            <v-menu offset-y>
                                <template v-slot:activator="{ on, attrs }">
                                    <v-btn color="error" icon class="mx-1" v-bind="attrs" v-on="on" >
                                        <v-icon> mdi-delete-outline </v-icon>
                                    </v-btn>
                                </template>
                                <v-card>
                                    <v-card-text>
                                        {{$store.getters.language.data.add_facts.delete_question}}
                                    </v-card-text>
                                    <v-card-actions>
                                        <v-spacer></v-spacer>
                                        <v-btn text class="elevation-0">
                                            {{$store.getters.language.data.add_facts.cancel_btn}}
                                        </v-btn>
                                        <v-btn color="error" class="elevation-0" @click="deleteAddFacts(item.add_fact_id)">
                                            {{$store.getters.language.data.add_facts.yes_btn}}
                                        </v-btn>
                                    </v-card-actions>
                                </v-card>
                            </v-menu>
                        </div>
                    </template>
                </v-data-table>
            </v-card-text>
            
            <v-card-actions v-if="selected_rows.length > 0">
                <v-btn color="error" @click="deleteAddFactsList">{{$store.getters.language.data.add_facts.delete_list_btn}}</v-btn>
            </v-card-actions>
                
        </v-card>
    </v-flex>
</v-layout>
</v-container>

<v-snackbar v-model="snackbar.value" :color="snackbar.color">
    {{snackbar.text}}
</v-snackbar>
</div>
</template>
<script>
    import requests from './../../requests/add_facts.request.js'
    import * as xlsx from 'xlsx/xlsx.js';
    
    export default {
        
        data() {
            return {
                add_facts: {},
                search: '',
                loading : true,
                loading_btn:false,
                selected_rows : [],
                snackbar: {
                    value: false,
                    text: '',
                    color: ''
                },
                rows:[],
                categories : [],
                selected_add_facts : {},
                headers: [

                    
                        { 
                            text: this.$store.getters.language.data.add_facts.add_facts_text,
                            align: 'start',
                            sortable: true,
                            value: 'add_facts_text',
                        },
                    {
                        text: this.$store.getters.language.data.categories.category_id,
                        align: 'start',
                        sortable: true,
                        value: 'category_id',
                    },
                        { 
                            text: this.$store.getters.language.data.add_facts.add_fact_review,
                            align: 'start',
                            sortable: true,
                            value: 'add_fact_review',
                        },
                        { 
                            text: this.$store.getters.language.data.add_facts.add_fact_date,
                            align: 'start',
                            sortable: true,
                            value: 'add_fact_date',
                        },   {
            text: this.$store.getters.language.data.actions,
            align: 'start',
            sortable: true,
            value: 'add_fact_id',
        }
                ],
            }
        },
        computed: {
            page(){
                const pageKey = 'ADDFACTS'
                return {}
            }
        },
        mounted(){
            this.readAddFacts();
        },
        methods: {
            exportExcel(){
                const list = this.rows
                let result = []
                for (let i = 0; i < list.length; i++) {
                    const item = list[i];
                    let obj = {add_fact_id : item.add_fact_id,add_facts_text : item.add_facts_text,category_id : item.category_id,add_fact_review : item.add_fact_review,add_fact_date : item.add_fact_date,
                    }
                    result.push(obj)
                }
				var f = xlsx.utils.json_to_sheet(result)
				var wb = xlsx.utils.book_new()
				xlsx.utils.book_append_sheet(wb, f, 'sheet')
				xlsx.writeFile(wb, result.length+' rows of '+'AddFacts.xlsx')
            },
            addAddFacts() {
                this.loading_btn = true
                requests.createAddFacts(this.add_facts).then(r => {
                    if (r.status == 200) {
                        this.add_facts = {}
                        this.rows.push(
                            r.data.new_data
                        )
                        this.snackbar = {
                            value: true,
                            text: 'AddFacts Added',
                            color: 'success'
                        }
                    } else {
                        this.snackbar = {
                            value: true,
                            text: 'Fail to add AddFacts',
                            color: 'error'
                        }
                    }
                })
                .finally(() => {
                    this.loading_btn = false
                })

            },
            deleteAddFacts(add_fact_id) {
                requests.deleteAddFacts(add_fact_id).then(r => {
                    if (r.status == 200) {
                        this.rows = this.rows.filter(f =>{
                            return f.add_fact_id != add_fact_id
                        })
                        this.snackbar = {
                            value: true,
                            text: 'AddFacts Deleted',
                            color: 'success'
                        }
                    } else {
                        this.snackbar = {
                            value: true,
                            text: 'Delete Faild',
                            color: 'error'
                        }
                    }
                })
            },
            
            deleteAddFactsList() {
                let ids = this.selected_rows.map(m => m.add_fact_id)
                requests.deleteAddFactsList(ids).then(r => {
                    if (r.status == 200) {
                        this.rows = this.rows.filter(f =>{  
                            return ids.indexOf(f.add_fact_id) == -1
                        })
                        this.snackbar = {
                            value: true,
                            text: this.selected_rows.length+' AddFacts Deleted',
                            color: 'success'
                        }
                    } else {
                        this.snackbar = {
                            value: true,
                            text: 'Delete Faild',
                            color: 'error'
                        }
                    }
                })
            },
            
            readAddFacts() {
                this.loading = true
                requests.getAllAddFacts().then(r => {
                    if (r.status == 200) {
                        this.rows = r.data.add_facts
                        
            this.categories = r.data.categories
            
                        this.loading = false
                    } else {
                        this.snackbar = {
                            value: true,
                            text: 'Fail to read AddFacts',
                            color: 'error'
                        }
                    }
                })
                .catch(e => {
                    this.snackbar = {
                        value: true,
                        text: 'Fail to read AddFacts',
                        color: 'error'
                    }
                })
                .finally(() => {
                    this.loading = false
                })
            },
            selectAddFacts(i) {
                this.selected_add_facts = i
                this.delete_dialog = true
            },
        },
    }
</script>
                    